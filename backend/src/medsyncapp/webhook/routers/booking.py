...
async def get_booking_notification_text(repo, booking_id):
    ...
    appointment_type_text = (
        f"👨‍⚕️ Shifokor: {hbold(booking_info.Doctor.full_name)}\n"
        if booking_info.Doctor
        else f"🔬 Diagnostika: {hbold(booking_info.Diagnostic.type_name)}\n"
    )

    return (
        f"🎉 Tabriklaymiz! Bron tasdiqlandi. 🎉\n\n"
        f"📋 Bron raqami: {hcode(booking_id)}\n"
        f"{appointment_type_text}"
        f"📆 Sana va vaqt: {hbold(booking_time)}\n\n"
        f"📍 Manzil: {hbold(booking_info.Location.name)}: {hbold(booking_info.Location.address)}\n\n"
        f"Xizmatimizni tanlaganingiz uchun rahmat! Savollaringiz bo‘lsa yoki vaqtni o‘zgartirmoqchi bo‘lsangiz, bemalol murojaat qiling. 📞"
    )
